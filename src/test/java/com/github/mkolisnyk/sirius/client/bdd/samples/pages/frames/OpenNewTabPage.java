...
public class OpenNewTabPage extends FramesAndWindowsCommonPage {

    @FindBy(locator = "//*[contains(text(), 'open a new tab in the same windo')]")
    public Control labelPrompt;
    @FindBy(locator = "link=Click Here")
    public Control buttonClickHere;

    public OpenNewTabPage(WebDriver driverValue) {
        super(driverValue);
    }
    ...
    public Page navigate() throws Exception {
        super.navigate();
        this.tabOpenNewTab.perform(click()).getParent().verify(current());
        return this;
    }
}
